        ...
        public static class Build {
            ...
            public NFile build() {
                ...
                if (file.isFile) {
                    ...
                    if (FileType.getInstance().getFileType(file.mimeType) == FileType.IMG) {
                        file.iconUrl = mFile.getAbsolutePath();
                    } else {
                        file.iconUrl = null;
                    }
                    //最后才build（只有文件时，才会build下载dir）
                    file.buildCorrectFileBean(FileServer.getInstance().getAddress(), FileServer.getInstance().getPort());
